from sqlalchemy.orm import Session
from db.models import Classroom, ClassroomMember
from uuid import UUID
from exceptions import errors


def get_classroom_members(db: Session, classroom_id: UUID):
    class_obj = db.get(Classroom, classroom_id)
    if class_obj is None:
        raise errors.NotFoundError(f"Classroom {classroom_id} found")
    return class_obj.members


def add_classroom_member(db: Session, classroom_id: UUID, user_id: UUID):
    class_member = ClassroomMember(classroom_id=classroom_id, user_id=user_id)
    db.add(class_member)
    db.commit()
    db.refresh(class_member)
    return class_member


def remove_classroom_member(db: Session, classroom_id: UUID, user_id: UUID):
    class_member = db.get(ClassroomMember, (classroom_id, user_id))
    if class_member is None:
        raise errors.NotFoundError(
            f"User {user_id} not found in classroom {classroom_id}"
        )
    db.delete(class_member)
    return class_member
